Here is the complete fixed `audit7.js` file. Copy and paste this entire code into your GitHub repository, replacing all existing content:
...
(function(){
  ...
  var P=[
    ...
    {n:'Health & Medical',d:'Medical history, directives, and emergency access information.',i:['HEALTH INSURANCE INFORMATION','MEDICAL RECORDS & HISTORY','PRESCRIPTION MEDICATIONS LIST','ADVANCE HEALTHCARE DIRECTIVE']},
    {n:'Legal & Estate',d:'Legal instruments, policy documentation, and estate planning records.',i:['LAST WILL & TESTAMENT','TRUST DOCUMENTATION','POWERS OF ATTORNEY','LIFE INSURANCE POLICIES','GUARDIANSHIP DESIGNATIONS']},
    {n:'Business Continuity',d:'Operational documentation for business owners, including transition planning.',i:['BUSINESS ENTITY DOCUMENTS','BUSINESS BANKING ACCESS','OPERATING OR PARTNERSHIP AGREEMENTS']},
  ...
  function naStyle(isNa){
    return 'display:inline-flex;align-items:center;justify-content:center;width:auto;min-width:42px;height:24px;padding:0 8px;flex-shrink:0;border:1px solid '+(isNa?'#c1b085':'#7A6842')+';border-radius:2px;background:'+(isNa?'rgba(193,176,133,0.08)':'transparent')+';box-shadow:'+(isNa?'0 0 12px rgba(193,176,133,0.5),0 0 24px rgba(193,176,133,0.2)':'none')+';cursor:pointer;transition:border-color 0.3s,background 0.3s,box-shadow 0.3s;';
  }
  ...
  function pillarHTML(pi){
    ...
    for(var ii=0;ii<6;ii++){
      ...
      rows+='<div id="r'+pi+'-'+ii+'" style="display:flex;align-items:center;gap:12px;padding:13px 16px;border:1px solid '+(on?'rgba(193,176,133,0.12)':'transparent')+';border-radius:2px;background:'+(on?'rgba(193,176,133,0.03)':'transparent')+';opacity:'+rowOpacity+';transition:border-color 0.3s,background 0.3s,opacity 0.3s;">'+
        ...
        '</div>' +
        '<div id="na' + pi + '-' + ii + '" onclick="__la.na(' + pi + ',' + ii + ')" style="' + naStyle(isNa) + '"><span style="font-family:Cinzel,serif;font-size:10px;letter-spacing:1.5px;font-weight:700;color:' + (isNa ? '#b8984e' : '#9a8d7a') + ';transition:color 0.3s;">N/A</span></div>' +
      '</div>';
    }
    ...
    var nextBtn;
    if (isP5) {
      nextBtn = '<button onclick="__la.p5()" style="font-family:Cinzel,serif;font-size:17px;font-weight:700;letter-spacing:3px;color:#100d0a;background:linear-gradient(135deg,#c1b085,#d4c4a0);border:none;padding:14px 36px;border-radius:2px;cursor:pointer;transition:opacity 0.3s;opacity:0.95;">CALCULATE</button>';
    } else if (isLast) {
      nextBtn = '<button onclick="__la.go(\'R\')" style="font-family:Cinzel,serif;font-size:17px;font-weight:700;letter-spacing:3px;color:#100d0a;background:linear-gradient(135deg,#c1b085,#d4c4a0);border:none;padding:14px 36px;border-radius:2px;cursor:pointer;transition:opacity 0.3s;opacity:0.95;">GET MY RESULTS</button>';
    } else {
      nextBtn = '<button onclick="__la.go(' + (pi + 2) + ')" style="font-family:Cinzel,serif;font-size:17px;font-weight:700;letter-spacing:3px;color:#100d0a;background:linear-gradient(135deg,#c1b085,#d4c4a0);border:none;padding:14px 36px;border-radius:2px;cursor:pointer;transition:opacity 0.3s;opacity:0.95;">NEXT PILLAR</button>';
    }
  ...
  function resultsHTML() {
    ...
    else if (percent >= 46) rec = 'Your planning has real complexity creating hidden risk. The gaps you are carrying could cost your family dozens of hours and real money.';
    ...
    return '<div style="max-width:600px;margin:0 auto;">' +
      ...
      '<div style="font-family:Georgia,serif;font-size:13px;color:#6b5a38;margin-bottom:24px;">You have ' + totalGaps + ' uncovered items across ' + pillarData.length + ' pillars. We will send the complete breakdown to your email.</div>' +
      ...
      '<div style="font-family:Bodoni Moda,serif;font-size:12px;font-style:italic;color:#8a7e6a;margin-top:8px;">"Order in Your Absence"</div></div></div>';
  }
  ...
  window.__la = {
    ...
    na: function(pi, ii) {
      ...
      if (naBtn) {
        naBtn.style.borderColor = isNa ? '#c1b085' : '#7A6842';
        ...
        var span = naBtn.querySelector('span'); if (span) span.style.color = isNa ? '#b8984e' : '#9a8d7a';
      }
      updateCtr(pi);
    },
    by: function() {
      ...
      if (y) { y.style.borderColor = '#c1b085'; y.style.color = '#c1b085'; y.style.background = 'rgba(193,176,133,0.05)'; y.style.boxShadow = '0 0 18px rgba(193,176,133,0.5),inset 0 0 12px rgba(193,176,133,0.05)'; }
    ...
    bn: function() {
      OB = false;
      var y = document.getElementById('by'), n = document.getElementById('bn'), h = document.getElementById('bh');
      if (n) { n.style.borderColor = '#c1b085'; n.style.color = '#c1b085'; n.style.background = 'rgba(193,176,133,0.05)'; n.style.boxShadow = '0 0 18px rgba(193,176,133,0.5),inset 0 0 12px rgba(193,176,133,0.05)'; }
